...
const NAV = [
  ...
  { to:'/chapters', icon:'📚', label:'Theo chương' },
      ...
      <div className="max-w-2xl mx-auto px-4">
        <div className="flex items-center justify-between h-14 gap-2">
          <div className="flex items-center gap-2 flex-shrink-0">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-green-400 to-teal-500 flex items-center justify-center text-white font-black text-lg shadow-md">T₇</div>
          ...
          <nav className="flex gap-0.5 overflow-x-auto flex-1 justify-center">
            {NAV.map(n => (
              <NavLink key={n.to} to={n.to} end={n.to=='/'}
                className={({ isActive }) =>
                  `px-2 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap relative flex-shrink-0 ${isActive?'bg-green-100 text-green-700':'text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}>
                <span>{n.icon}</span>
                <span className="hidden md:inline ml-1">{n.label}</span>
                {n.to === '/review' && wrongCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-black">
          ...
          <div className="flex-shrink-0 text-right">
            <div className="flex items-center gap-1 mb-0.5">
              ...
              <span className="text-xs text-yellow-500">⭐{stats.xp}</span>
            </div>
            <div className="xp-track w-16">
